...
A PDFA *A* over $\large\Sigma$ is a tuple $\large (Q, q_{\text{in}}, \pi, \tau)$, where $\large \pi : Q \to \Delta(\Sigma_{\$})$ associates a probability distribution over $\large \Sigma_{\$}$ with each state in $\large Q$. The function $\large \tau : Q \times \Sigma \to \Delta(Q)$ is a probabilistic transition function, mapping each state and symbol to a probability distribution over the states.
