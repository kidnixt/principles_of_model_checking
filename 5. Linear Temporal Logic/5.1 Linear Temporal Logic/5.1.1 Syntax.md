...
$\LARGE \Diamond$ ("eventually", sometimes in the future)
...
$\LARGE \Diamond \varphi$ ensures that $\LARGE \varphi$ will be true eventually in the future
$\LARGE \square \varphi$ is satisfied if and only if it is not the case that eventually $\LARGE \neg\varphi$ holds. This is equivalent to the fact that $\LARGE \varphi$ holds from now on forever 