...
$$
The requirement *"once red, the light always become green eventually afte\textbf{Always Eventually $a$ (infinitely often):} This property ensures that at any point in time, there is always a future moment when state $a$ will be reached. In other words, no matter when the system is observed, there will always be a future point where $a$ \textbf{occurs}. This implies that state $a$ occurs \textit{infinitely often}. Even if $a$ is not happening now, it will appear repeatedly in the future. \\

\textbf{Eventually Always $a$ (eventually forever):} This property states that at some future point, the system will reach a state where it will \textbf{always} remain in state $a$. In other words, the system will eventually enter state $a$, and from that moment onward, it will \textit{never} leave $a$ again.r being yellow for some time"* is expressed by
